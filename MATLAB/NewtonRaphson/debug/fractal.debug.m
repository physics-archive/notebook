...
func = @(Z) (Z-Adjustment).^3-1+Adjustment;
dfunc = @(Z) 3*(Adjustment-Z).^2;

%Obtain the roots of func (func(root) = 0)
%reRoots = fzero(func,0,); Replaced by roots
%syms v
%V = coeffs(func(v),v,'All');
Roots = roots([1-Adjustment 0 0 -1+Adjustment]);

Nx = 1200;
Ny = 1200;

%ITERATE EXPORT_FIG
for scope = 0.1:0.05:1 
    index = 100*scope;
    ...
    export_fig(strcat(['/Users/s285/Downloads/NewtonZoom/newtonzoom','-frame-',num2str(index)]), '-png', '-r200');
