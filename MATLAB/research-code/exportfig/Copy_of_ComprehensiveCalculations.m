...
Data = 'm20s'; %The velroot function will generate an array using this file
AmplifierGain = 10; %10 V/V setting on the P31 Amplifier
ProbeRate = 10; %On ODS Oscilloscope software - the default setting
SampleRate = 250; %5000 signals per log - variable time period
...
examplePSD = dat(exampleDat); %m^2/s^4/hz units
exampleSqrtPSD_SI = examplePSD./(sqrt(2)*2*pi*F); %m/s/sqrt(hz)-rms units
example_squaredVelocity = zeros(30,1);

for j = octRange
    for k = kRange
        if F(k) > octaveLow(j) && F(k) <= octaveHigh(j)
            example_squaredVelocity(j) = example_squaredVelocity(j) + exampleSqrtPSD_SI(k)^2*bandwidth;
        end
